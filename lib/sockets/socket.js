...
module.exports.init = function(io){
  ...
  io.sockets.on('connection', function(socket){
    ...
    setTimeout(function(){
      socket.emit('Create world', playerIo.grid, playerIo.players);
      socket.broadcast.to(socket.room).emit('Create world', playerIo.grid, playerIo.players);
    }, 200);
    ...
    socket.on('Move zombie', playerIo.moveZombie(socket).bind(playerIo));
